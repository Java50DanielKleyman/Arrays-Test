...
public class ArraysInt {
	...
	public static int binarySearch(int[] array, int number) {
		...
		int middle = right / 2;
		while (left <= right) {			
			if(middle == 0 && number == array[middle]) {
				return middle;
			} else if(middle > 0 && number == array[middle] && number != array[middle - 1]) {
				return middle;
			} else if (middle > 0 && number < array[middle] || (number == array[middle] && number == array[middle - 1])) {
				right = middle - 1;
			} else {
				left = middle + 1;
			}
